...
`DATABASE_URL`, `BATCH_AUTH_TOKEN`, and `CORS_ALLOWED_ORIGINS` are injected as App Runner environment variables sourced from **AWS Systems Manager Parameter Store** — they are not set in this repo, not in `docker-compose.yml`, and not in any committed file.
